...
function ajouterCategorieOption(nom) {
    const conteneur = document.getElementById("options-categorie");
    const valeur = nom.toLowerCase().replace(/\s+/g, "-").normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const option = document.createElement("div");
    option.className = "selecteur-option";
    option.dataset.valeur = valeur;
...
if (btnConfirmerCategorie) {
    btnConfirmerCategorie.addEventListener("click", () => {
        const nom = inputNouvelleCategorie.value.trim();
        if (nom) {
            ajouterCategorieOption(nom);
            modaleCategorie.classList.remove("visible");
        }
    });
}
