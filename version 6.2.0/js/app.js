...
const stockGlobalActuel = 156; // Valeur démo
...
function selectionnerPeriodeHistorique(periode) {
    ...
    // Mettre à jour le texte du bouton date-select
...
function appliquerDatesPersonnaliseesHistorique() {
    ...
    // Décocher toutes les options fixes
...
<div class="historique-quantites">
    ...
    <span class="quantite-badge variation ${(h.apres - h.avant) > 0 ? 'positif' : (h.apres - h.avant) < 0 ? 'negatif' : 'neutre'}">${(h.apres - h.avant) > 0 ? '+' : ''}${h.apres - h.avant}</span>
...
document.getElementById('scanner-qr-input-galerie').addEventListener('change', (event) => {
    ...
    event.target.value = '';
            ...
            if (listeEmplacementMouvement) {
                listeEmplacementMouvement.querySelectorAll(".selecteur-option-mouvement").forEach(option => {
                    option.addEventListener("click", () => {
                        listeEmplacementMouvement.querySelectorAll(".selecteur-option-mouvement").forEach(o => o.classList.remove("active"));
                        option.classList.add("active");

                        const nom = option.querySelector(".option-emp-nom-mouvement").textContent;
                        const qty = option.dataset.qty;
                        document.querySelector(".emplacement-nom-mouvement").textContent = nom;
                        document.querySelector(".emplacement-qty-mouvement").textContent = qty + " unités";
...
document.querySelectorAll(".option-periode-historique").forEach(option => {
    option.addEventListener("click", () => {
        ...
        if (periode === "periode") {
            ...
            selectionnerPeriodeHistorique(periode);
        }
    });
});
...
if (btnAppliquerDatesHistorique) {
    btnAppliquerDatesHistorique.addEventListener("click", () => {
        appliquerDatesPersonnaliseesHistorique();
    });
}
