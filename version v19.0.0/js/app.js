...
async function sauvegarderSessionCache(profil, entreprise, role) {
    try {
        await depotSession.setItem('profil', profil);
        await depotSession.setItem('entreprise', entreprise);
        await depotSession.setItem('role', role);
    } catch (e) {
        console.error('Erreur sauvegarde session cache :', e);
    }
}
...
async function viderCacheComplet() {
    try {
        await depotDonnees.clear();
        await depotSession.clear();
    } catch (e) {
...
function mettreAJourIndicateurCloud() {
    // Chercher l'item Cloud dans le menu via sa position dans la section Données
    const sectionsMenu = document.querySelectorAll('#ecran-menu .menu-section');
    // La section "Données" est la 3ème section (index 2)
    if (sectionsMenu.length >= 3) {
        const sectionDonnees = sectionsMenu[2];
        const items = sectionDonnees.querySelectorAll('.menu-item');
        // Cloud est le 3ème item de la section Données (index 2)
        if (items.length >= 3) {
            const itemCloud = items[2];
            const indicateur = itemCloud.querySelector('.menu-item-droite span');
            if (indicateur) {
                if (navigator.onLine) {
...
async function synchroniserDonnees() {
    ...
    if (!utilisateurActuel || !entrepriseActuelle) return;
    ...
    try {
        // 1. Recharger le contexte utilisateur
        await chargerContexteUtilisateurComplet();
        await chargerEntreprisesUtilisateur();

        // 2. Sauvegarder la session dans le cache
        await sauvegarderSessionCache(profilActuel, entrepriseActuelle, roleUtilisateurActuel);

        // 3. Recharger et mettre en cache toutes les données
        await chargerEtCacherProduits();
        await chargerEtCacherCategories();
        await chargerEtCacherNotifications();
        await chargerEtCacherStatistiquesAccueil();
        await chargerEtCacherMouvementsRecents();
        await chargerEtCacherMembres();
        await chargerEtCacherCompteursRapports();
        await chargerEtCacherTotauxBudgetaires();
...
async function chargerEtCacherProduits() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.PRODUITS + '_' + entrepriseActuelle.id, produitsEntreprise);
    } catch (e) {
...
async function chargerEtCacherCategories() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.CATEGORIES + '_' + entrepriseActuelle.id, categoriesEntreprise);
    } catch (e) {
...
async function chargerEtCacherNotifications() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.NOTIFICATIONS + '_' + entrepriseActuelle.id, notificationsStock);
    } catch (e) {
...
async function chargerEtCacherStatistiquesAccueil() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.STATISTIQUES_ACCUEIL + '_' + entrepriseActuelle.id, statistiquesAccueil);
    } catch (e) {
...
async function chargerEtCacherMouvementsRecents() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.MOUVEMENTS_RECENTS + '_' + entrepriseActuelle.id, mouvementsRecentsAccueil);
    } catch (e) {
...
async function chargerEtCacherMembres() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.MEMBRES + '_' + entrepriseActuelle.id, membresEntrepriseActuelle);
    } catch (e) {
...
async function chargerEtCacherCompteursRapports() {
    ...
    try {
        ...
        await sauvegarderDansCache(CLES_CACHE.COMPTEURS_RAPPORTS + '_' + entrepriseActuelle.id, compteursRapportsRapport);
        await sauvegarderDansCache(CLES_CACHE.TOTAUX_BUDGETAIRES + '_' + entrepriseActuelle.id, totauxBudgetairesRapport);
    } catch (e) {
...
async function restaurerDonneesDepuisCache() {
    if (!entrepriseActuelle?.id) return false;

    const idEntreprise = entrepriseActuelle.id;
    let restaure = false;

    try {
        // Produits
        const produitsCaches = await lireDepuisCache(CLES_CACHE.PRODUITS + '_' + idEntreprise);
        if (produitsCaches && Array.isArray(produitsCaches)) {
            produitsEntreprise = produitsCaches;
            restaure = true;
        }

        // Catégories
        const categoriesCachees = await lireDepuisCache(CLES_CACHE.CATEGORIES + '_' + idEntreprise);
        if (categoriesCachees && Array.isArray(categoriesCachees)) {
            categoriesEntreprise = categoriesCachees;
        }

        // Notifications
        const notificationsCachees = await lireDepuisCache(CLES_CACHE.NOTIFICATIONS + '_' + idEntreprise);
        if (notificationsCachees && Array.isArray(notificationsCachees)) {
            notificationsStock = notificationsCachees;
        }

        // Statistiques accueil
        const statistiquesCachees = await lireDepuisCache(CLES_CACHE.STATISTIQUES_ACCUEIL + '_' + idEntreprise);
        if (statistiquesCachees) {
            statistiquesAccueil = statistiquesCachees;
        }

        // Mouvements récents
        const mouvementsCaches = await lireDepuisCache(CLES_CACHE.MOUVEMENTS_RECENTS + '_' + idEntreprise);
        if (mouvementsCaches && Array.isArray(mouvementsCaches)) {
            mouvementsRecentsAccueil = mouvementsCaches;
        }

        // Membres
        const membresCaches = await lireDepuisCache(CLES_CACHE.MEMBRES + '_' + idEntreprise);
        if (membresCaches && Array.isArray(membresCaches)) {
            membresEntrepriseActuelle = membresCaches;
        }

        // Compteurs rapports
        const compteursCaches = await lireDepuisCache(CLES_CACHE.COMPTEURS_RAPPORTS + '_' + idEntreprise);
        if (compteursCaches) {
            compteursRapportsRapport = compteursCaches;
        }

        // Totaux budgétaires
        const totauxCaches = await lireDepuisCache(CLES_CACHE.TOTAUX_BUDGETAIRES + '_' + idEntreprise);
        if (totauxCaches) {
            totauxBudgetairesRapport = totauxCaches;
        }

        if (restaure) {
            console.log('Données restaurées depuis le cache IndexedDB');
        }
    } catch (e) {
...
async function restaurerSessionDepuisCache() {
    const session = await lireSessionCache();

    if (session.profil) {
        profilActuel = session.profil;
    }
    if (session.entreprise) {
        entrepriseActuelle = session.entreprise;
    }
    if (session.role) {
        roleUtilisateurActuel = session.role;
    }
...
async function initialiserHorsLigne() {
    const etat = lireEtatAppli();
    const ecranInitial = window.__SMV_ECRAN_INITIAL || 'ecran-langue';

    console.log('Initialisation offline-first — état :', etat);
    console.log('Écran initial déterminé :', ecranInitial);

    // Si l'utilisateur a une entreprise choisie, restaurer le cache immédiatement
    if (etat && etat.etape === ETAPES_APPLI.ENTREPRISE_CHOISIE && etat.entrepriseId) {
        // Restaurer la session depuis IndexedDB
        const sessionRestoree = await restaurerSessionDepuisCache();

        if (sessionRestoree) {
            console.log('Session restaurée depuis le cache');

            // Restaurer les données
            await restaurerDonneesDepuisCache();

            // Remplir l'interface avec les données cachées
            remplirInterfaceUtilisateurEtEntreprise();
    ...
    // Afficher l'écran initial (déjà déterminé par le script synchrone)
    afficherEcran(ecranInitial);

    // Initialiser le système de traduction
    await initialiserTraduction();

    // Mettre à jour l'indicateur Cloud
    setTimeout(mettreAJourIndicateurCloud, 100);
}
...
let detecteurPretLecteur = false; // Flag : détecteur initialisé
    ...
    if (ecran) {
        ...
        // Sauvegarder dans sessionStorage (pour les retours focus/visibilitychange)
        sessionStorage.setItem("ecran-actuel", idEcran);

        // Sauvegarder dans l'état persistant (pour le prochain lancement)
    ...
    // Mettre à jour l'indicateur cloud à chaque changement d'écran
    mettreAJourIndicateurCloud();
}
...
async function validerMouvement() {
    ...
    try {
        ...
        // Message de succès
        const cleMessage = typeMouvementActuel === "entree" ? "entreeEnregistree" : typeMouvementActuel === "sortie" ? "sortieEnregistree" : "inventaireEnregistre";
afficherMessageSucces(tm(cleMessage));

        // Mettre en cache les données mises à jour
        await sauvegarderDansCache(CLES_CACHE.PRODUITS + '_' + entrepriseActuelle.id, produitsEntreprise);
        await sauvegarderDansCache(CLES_CACHE.NOTIFICATIONS + '_' + entrepriseActuelle.id, notificationsStock);
        await sauvegarderDansCache(CLES_CACHE.STATISTIQUES_ACCUEIL + '_' + entrepriseActuelle.id, statistiquesAccueil);
        await sauvegarderDansCache(CLES_CACHE.MOUVEMENTS_RECENTS + '_' + entrepriseActuelle.id, mouvementsRecentsAccueil);
...
async function enregistrerProduit() {
    // Si on est en mode modification, déléguer à la fonction de modification
    ...
    try {
        ...
        let photoUrl = null;
        let photoBase64Finale = null;
        if (donnees.photoBase64 && donnees.photoBase64.startsWith("data:")) {
            photoUrl = await uploaderPhotoProduit(donnees.photoBase64);
            // Compresser en base64 pour affichage instantané
            photoBase64Finale = await convertirPhotoProduitEnBase64(donnees.photoBase64);
        }
...
const { data, error } = await clientSupabase
    .rpc("enregistrer_produit", {
        ...
        p_photo_base64: photoBase64Finale,
...
async function ouvrirFormulaireModifierProduit() {
    ...
    if (produit.categorie_id) {
        const categorieCorrespondante = categoriesEntreprise.find(c => c.id === produit.categorie_id);
        if (categorieCorrespondante && texteCategorie && selecteurCategorie) {
            texteCategorie.textContent = categorieCorrespondante.nom;
            texteCategorie.classList.add("selectionne");
            selecteurCategorie.dataset.valeur = categorieCorrespondante.id;

            // Marquer l'option comme active dans la liste
            const options = document.querySelectorAll("#options-categorie .selecteur-option");
            options.forEach(opt => {
                opt.classList.toggle("actif", opt.dataset.valeur === categorieCorrespondante.id);
            });
        }
    }
...
/**
 * Collecte les données du formulaire et effectue la modification via RPC.
 */
async function executerModifierProduit() {
    ...
    // Validation
    if (!donnees.nom) {
        afficherMessageErreur(tm('nomProduitRequis'));
        return;
    }

    if (donnees.emplacements.length === 0) {
        afficherMessageErreur(tm('emplacementRequis'));
        return;
    }

    if (!produitIdModifierProduit) {
        afficherMessageErreur("Erreur : aucun produit à modifier.");
        return;
    }
    ...
    const btnEnregistrer = document.querySelector("#ecran-ajout-produit .ajout-enregistrer-btn");
    const texteOriginal = btnEnregistrer.textContent;
    btnEnregistrer.textContent = "Modification...";
    btnEnregistrer.disabled = true;

    try {
        // Gérer la photo
        let photoUrlFinale = photoUrlExistanteModifierProduit;

        const apercuPhoto = document.getElementById("apercu-final-photo-produit");
        const photoSrc = (apercuPhoto && apercuPhoto.style.display !== "none" && apercuPhoto.src) ? apercuPhoto.src : null;
        ...
        if (!photoSrc || photoSrc === "") {
            ...
            photoUrlFinale = null;
            photoBase64Finale = null;
        } else if (photoSrc.startsWith("data:")) {
            // Nouvelle photo (base64) → upload + stocker base64
            photoUrlFinale = await uploaderPhotoProduit(photoSrc);
            photoBase64Finale = await convertirPhotoProduitEnBase64(photoSrc);
        } else {
            // Photo URL existante inchangée → garder le base64 existant
            photoBase64Finale = produitActuelDetails?.photo_base64 || null;
        }

        // Sinon, c'est l'URL existante (inchangée), on garde photoUrlFinale tel quel
        ...
        // Appel RPC modification
                const { data, error } = await clientSupabase
            .rpc("modifier_produit_complet", {
                p_produit_id: produitIdModifierProduit,
                p_nom: donnees.nom,
                p_code: donnees.code || null,
                p_code_barres: donnees.code || null,
                p_format_code_barres: formatCodeBarresFinal || null,
        ...
        if (error) {
            ...
            afficherMessageErreur("Impossible de modifier le produit : " + (error.message || "Erreur inconnue"));
            return;
        }

        console.log("Produit modifié :", data);

                afficherMessageSucces(tm('produitModifie'));

        // Sauvegarder l'ID du produit AVANT de réinitialiser le mode
        const idProduitModifie = produitIdModifierProduit;
        ...
        // Retourner aux détails du produit modifié pour voir les changements
        await afficherDetailsProduit(idProduitModifie);


    } catch (erreur) {
        console.error("Erreur inattendue modification :", erreur);
        afficherMessageErreur("Une erreur est survenue lors de la modification.");
    } finally {
        btnEnregistrer.textContent = texteOriginal;
        btnEnregistrer.disabled = false;
    }
...
async function afficherDetailsProduit(produitId) {
    ...
    if (zoneImage) {
        const photoSource = obtenirSourceImage(produit.photo_base64, produit.photo_url);
        if (photoSource) {
...
if (codeBarresDetail && formatEffectifDetail) {
    codeBarresDetail = completerCodeAvecControle(codeBarresDetail, formatEffectifDetail);
        ...
        const btnModifierModifierProduit = contenusDetails.querySelector(".details-modifier-btn");
        if (btnModifierModifierProduit) {
            btnModifierModifierProduit.addEventListener("click", async () => {
...
/**
 * FONCTION PRINCIPALE — Génère et télécharge le PDF professionnel
 */
async function exporterRapportPDF() {
    ...
    try {
        ...
        // 4) PIED DE RAPPORT (signatures, mentions)
        ...
        var definitionDocument = {
            ...
            // FOND DÉCORATIF — Barre latérale sur chaque page
            background: function (pageCourante, taillesPage) {
                return [
                    // Barre latérale gauche couleur thème
                    {
                        canvas: [
                            {
                                type: "rect",
            ...
            // EN-TÊTE — Logo + nom sur chaque page (sauf couverture)
            header: function (pageCourante, totalPages, taillesPage) {
                if (pageCourante === 1) return null; // Pas d'en-tête sur la couverture
            ...
            footer: function (pageCourante, totalPages) {
                if (pageCourante === 1) return null; // Pas de pied sur la couverture
        ...
        pdfMake.createPdf(definitionDocument).download(nomFichier, function () {
            ...
            afficherMessageSucces("Rapport PDF exporté avec succès !");
        });
...
async function connecterAvecGoogle() {
    // IMPORTANT : sauvegarder que l'utilisateur est en cours d'authentification
    // Pour que le retour depuis Google ne montre pas l'écran auth
    mettreAJourEtatAppli({
        enCoursAuthentification: true
    });

    // Sauvegarder l'écran choix entreprise comme destination après auth
    sessionStorage.setItem("ecran-actuel", "ecran-choix-entreprise");
    ...
    if (error) {
        ...
        // Retirer le flag
        mettreAJourEtatAppli({ enCoursAuthentification: false });
    }
}


async function deconnecterUtilisateur() {
    // Marquer la déconnexion AVANT tout pour l'anti-flash
    marquerDeconnexion();
    ...
    // Vider le cache des données (mais garder l'état de navigation)
...
async function initialiserSessionUtilisateur(session, options = {}) {
    ...
    console.log("Initialisation session :", sessionActuelle ? "session active" : "pas de session");
    ...
    if (!utilisateurActuel) {
        ...
        // Si l'état dit onboarding_termine, montrer auth
        // Sinon, le script synchrone a déjà déterminé l'écran
        const etat = lireEtatAppli();
        if (etat && etat.etape === ETAPES_APPLI.ONBOARDING_TERMINE) {
            afficherEcran("ecran-auth");
        } else if (!etat || etat.etape === ETAPES_APPLI.NEUF) {
            // Ne rien faire, l'écran initial est déjà affiché
        }
        return;
    }
    ...
    marquerEtapeAuthentifie(
        ...
        utilisateurActuel.user_metadata?.full_name || ''
    );
    ...
    if (navigator.onLine) {
        ...
        try {
            ...
            // Sauvegarder les entreprises dans le cache
            await sauvegarderDansCache(CLES_CACHE.ENTREPRISES_LISTE, entreprisesUtilisateur);

            remplirInterfaceUtilisateurEtEntreprise();
            rendreListeChoixEntreprise();

            // Charger et cacher toutes les données
            await chargerEtCacherCategories();
            await chargerEtCacherProduits();
            await chargerEtCacherNotifications();
            rendreNotificationsStock();
            ...
            console.error("Erreur chargement en ligne, tentative cache :", erreur);
        ...
        const sessionRestoree = await restaurerSessionDepuisCache();
        if (sessionRestoree) {
            // Restaurer la liste des entreprises aussi
            const entreprisesCachees = await lireDepuisCache(CLES_CACHE.ENTREPRISES_LISTE);
            if (entreprisesCachees) {
                entreprisesUtilisateur = entreprisesCachees;
            }
    ...
    if (restaurerEcran) {
        // Essayer de restaurer l'écran sauvegardé dans sessionStorage
        const ecranSession = sessionStorage.getItem("ecran-actuel");
        if (ecranSession && document.getElementById(ecranSession)) {
            afficherEcran(ecranSession);
            return;
        }

        // Sinon, utiliser l'état persistant
        const etat = lireEtatAppli();
        if (etat && etat.dernierEcran && document.getElementById(etat.dernierEcran)) {
            // Vérifier que l'écran est cohérent avec l'état
            if (etat.etape === ETAPES_APPLI.ENTREPRISE_CHOISIE && entrepriseActuelle) {
                afficherEcran(etat.dernierEcran);
                return;
            }
        }
    }
...
async function verifierRedirectionUtilisateur() {
    // D'abord, initialiser le mode hors-ligne (affiche l'écran depuis le cache)
    await initialiserHorsLigne();
    ...
    if (navigator.onLine) {
        try {
            ...
            if (donneesSession.session) {
                ...
                // Sinon, l'écran initial (langue) est déjà affiché
            }
        } catch (erreur) {
            console.error("Erreur vérification session :", erreur);
            // En cas d'erreur réseau, les données du cache sont déjà affichées
        }
    } else {
        // Hors-ligne : les données du cache sont déjà affichées par initialiserHorsLigne()
        console.log("Démarrage hors-ligne — données du cache affichées");
    }
}
...
const scannerQrEntreprise = document.getElementById('scanner-qr-entreprise');
        ...
        document.addEventListener("DOMContentLoaded", () => {
              // Afficher l'écran initial IMMÉDIATEMENT (déterminé par le script synchrone)
    const ecranInit = window.__SMV_ECRAN_INITIAL || 'ecran-langue';
    const ecranElement = document.getElementById(ecranInit);
    if (ecranElement) {
        ecranElement.classList.add('actif');
        ecranActuel = ecranInit;

        // Afficher la nav si nécessaire
        const nav = document.getElementById("nav-principale");
        const ecransAvecNav = ["ecran-accueil", "ecran-produits", "ecran-historique", "ecran-rapports", "ecran-menu"];
        if (nav) {
            nav.style.display = ecransAvecNav.includes(ecranInit) ? "flex" : "none";
        }
    }

    // Appliquer le mode sombre si nécessaire
    const modePref = lireModePreference();
    if (modePref === 'sombre') {
    ...
    // Lancer la vérification complète (async, ne bloque pas l'affichage)
...
if (btnChoixEntrepriseContinuer) {
    btnChoixEntrepriseContinuer.addEventListener("click", async () => {
        ...
        // Marquer l'étape entreprise choisie
        if (entrepriseActuelle) {
            sauvegarderThemeEntreprise(entrepriseActuelle.theme);
            marquerEtapeEntrepriseChoisie(entrepriseActuelle.id, entrepriseActuelle.nom);
        }
...
document.querySelectorAll(".nav-item").forEach(item => {
    item.addEventListener("click", async () => {
        ...
        // Charger et cacher les données si en ligne
        if (ecranCible === "ecran-historique" && navigator.onLine) {
            await chargerHistoriqueMouvements();
            if (entrepriseActuelle?.id) {
                await sauvegarderDansCache(CLES_CACHE.HISTORIQUE + '_' + entrepriseActuelle.id, donneesHistorique);
            }
        } else if (ecranCible === "ecran-historique" && !navigator.onLine) {
            // Restaurer l'historique depuis le cache
            const historiqueCache = await lireDepuisCache(CLES_CACHE.HISTORIQUE + '_' + entrepriseActuelle?.id);
            if (historiqueCache) {
                donneesHistorique = historiqueCache;
                rendreHistorique();
            }
        }

        if (ecranCible === "ecran-rapports" && navigator.onLine) {
            await chargerDonneesEcranRapportsRapport();
            if (entrepriseActuelle?.id) {
                await sauvegarderDansCache(CLES_CACHE.COMPTEURS_RAPPORTS + '_' + entrepriseActuelle.id, compteursRapportsRapport);
                await sauvegarderDansCache(CLES_CACHE.TOTAUX_BUDGETAIRES + '_' + entrepriseActuelle.id, totauxBudgetairesRapport);
        ...
        if (ecranCible === "ecran-accueil" && navigator.onLine) {
            await chargerDonneesAccueil();
            if (entrepriseActuelle?.id) {
                await sauvegarderDansCache(CLES_CACHE.STATISTIQUES_ACCUEIL + '_' + entrepriseActuelle.id, statistiquesAccueil);
                await sauvegarderDansCache(CLES_CACHE.MOUVEMENTS_RECENTS + '_' + entrepriseActuelle.id, mouvementsRecentsAccueil);
...
if (menuEntreprise) {
    menuEntreprise.addEventListener("click", async () => {
        ...
        if (navigator.onLine) {
            // Charger et cacher les membres dynamiquement
            await chargerMembresEntreprise();
            if (entrepriseActuelle?.id) {
                await sauvegarderDansCache(CLES_CACHE.MEMBRES + '_' + entrepriseActuelle.id, membresEntrepriseActuelle);
            }
        } else {
            // Restaurer les membres depuis le cache
            const membresCaches = await lireDepuisCache(CLES_CACHE.MEMBRES + '_' + entrepriseActuelle?.id);
            if (membresCaches) {
                membresEntrepriseActuelle = membresCaches;
            }
        }
...
if (btnAppliquerDatesRapport) {
    ...
    if (!document.hidden) {
        const ecranSauvegarde = sessionStorage.getItem("ecran-actuel");
        if (ecranSauvegarde && ecranSauvegarde !== ecranActuel && document.getElementById(ecranSauvegarde)) {
...
window.addEventListener("focus", () => {
    ...
    console.log("Changement auth :", evenement);

    setTimeout(async () => {
        if (evenement === "SIGNED_OUT") {
            // La déconnexion est gérée par deconnecterUtilisateur()
            // Mais au cas où c'est un signOut externe :
            const etat = lireEtatAppli();
            if (!etat || etat.etape !== ETAPES_APPLI.ONBOARDING_TERMINE) {
                marquerDeconnexion();
            }
        ...
        if (evenement === "SIGNED_IN") {
            // IMPORTANT : après retour de Google OAuth
            // Marquer authentifié IMMÉDIATEMENT pour l'anti-flash
            if (session?.user) {
                marquerEtapeAuthentifie(
                    session.user.id,
                    session.user.email,
        ...
        // INITIAL_SESSION, etc.
        // Ne rien faire, verifierRedirectionUtilisateur gère déjà
    }, 0);
});
