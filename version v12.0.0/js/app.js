...
let instanceScannerLecteurCodeBarres = null; // Instance Html5Qrcode
...
/**
 * Démarre la caméra arrière et lance le scan continu
 */
async function demarrerCameraLecteurCodeBarres() {
    ...
    try {
        ...
        // Démarrer le scan continu avec Html5Qrcode
...
/**
 * Lance le scan continu frame par frame via un canvas caché
 */
function lancerScanContinuLecteurCodeBarres(videoElement) {
    if (!videoElement || !Html5Qrcode) return;

    scanEnCoursLecteurCodeBarres = true;

    const scannerInterne = new Html5Qrcode("conteneur-cache-lecteur-code-barres", /* verbose= */ false);

    // Créer un conteneur caché si n'existe pas
    let conteneurCache = document.getElementById("conteneur-cache-lecteur-code-barres");
    if (!conteneurCache) {
        conteneurCache = document.createElement("div");
        conteneurCache.id = "conteneur-cache-lecteur-code-barres";
        conteneurCache.style.display = "none";
        document.body.appendChild(conteneurCache);
    }

    const canvasCapture = document.createElement("canvas");
    const ctxCapture = canvasCapture.getContext("2d", { willReadFrequently: true });

    const scannerFrame = async () => {
        ...
        if (videoElement.readyState >= videoElement.HAVE_ENOUGH_DATA) {
            ...
            try {
                ...
                const resultatScan = await scannerInterne.scanFileV2(fichier, /* showImage= */ false);

                if (resultatScan && resultatScan.decodedText) {
                    gererCodeDetecteLecteurCodeBarres(resultatScan.decodedText);
                    return; // Arrêter le scan après détection
                }
            } catch (e) {
                // Pas de code trouvé dans cette frame — continuer
            }
        }

        if (scanEnCoursLecteurCodeBarres) {
            requestAnimationFrame(scannerFrame);
        }
    };

    // Attendre un petit délai puis commencer le scan
    setTimeout(() => {
        if (scanEnCoursLecteurCodeBarres) {
            scannerFrame();
        }
...
function gererCodeDetecteLecteurCodeBarres(codeDetecte) {
    ...
    // Appliquer le résultat après un court délai
...
function appliquerResultatScanLecteurCodeBarres(code) {
    ...
    if (origine === "ajout-produit") {
        ...
        if (inputCode) {
            ...
            // Détecter le format et mettre à jour l'étiquette
            formatSelectionneCodeProduit = detecterFormatCodeProduit(code) || "AUTO";
            mettreAJourEtiquetteCodeProduit();
        }
    } else if (origine === "produits") {
        // Rechercher le produit par code-barres dans la liste
...
/**
 * Scan un code-barres depuis une image importée
 */
async function scannerImageLecteurCodeBarres(fichier) {
    ...
    try {
        const scanner = new Html5Qrcode("conteneur-cache-lecteur-code-barres", /* verbose= */ false);
        const resultat = await scanner.scanFileV2(fichier, /* showImage= */ false);

        if (resultat && resultat.decodedText) {
            gererCodeDetecteLecteurCodeBarres(resultat.decodedText);
        } else {
            if (indication) {
